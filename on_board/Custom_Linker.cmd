...
MEMORY
{
    PAGE 0:
        ...
        SARAM9 : origin = 003c000h, length = 0004000h
        SARAM10 : origin = 0040000h, length = 000C000h	/* For DUT sandbox to prevent weird program behavior */
        SARAM11 (RWX): origin = 004C000h, length = 0004000h	/* For the corpus sandbox so that it will not effect program memory*/
...
SECTIONS
{
        ...
        .const   : {} > SARAM8  PAGE 0        
        .cio     : {} > DARAM2  PAGE 0
        .program_sandbox : > SARAM10 /* Store the device under test here */
        {
        	--library=*ximagex.lib<Boundarymn.obj>(.text)
        }
        .data_sandbox : {} > SARAM11 				   /* Data may get muddled because of the SUT having a error */
}
