...
#define WIDTH 250 //Fixed size width for now.
 ...
 ********************************************************/
    ...
    mutator(current_input,100);
    test_case_size = current_input[0]%100;
    ...
    // pace_test(current_input,test_case_size);
    // telecom_test(current_input, test_case_size);    
    // sonar_test(current_input, test_case_size);
    process_image(current_input,current_input[0]%100);
