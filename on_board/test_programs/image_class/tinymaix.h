...
#define TM_ALIGN_SIZE   (16)     //8 byte align
...
#define TM_MATP(mat,y,x,ch) ((mat)->data + ((y)*(mat)->w + (x))*(mat)->c + (ch))
    ...
    #endif
    typedef uint16_t mtype_t;    //mat data type
    typedef uint16_t wtype_t;    //weight data type
    typedef uint16_t btype_t;    //bias data type
    typedef float sumtype_t;    //sum data type 
    typedef float zptype_t;     //zeropoint data type
#else 
...
typedef struct{
    uint32_t magic;         //"MAIX"
    uint16_t  mdl_type;      //0 int8, 1 int16, 2 fp32,
    uint16_t  out_deq;       //0 don't dequant out; 1 dequant out
    ...
    uint16_t out_dims[4];
    uint16_t  reserve[28];   //reserve for future
    uint16_t  layers_body[0];//oft 64 here
}tm_mdlbin_t;
...
typedef struct{
    ...
    uint16_t* buf;           //main buf addr
    uint16_t* subbuf;        //sub buf addr
    uint16_t main_alloc;    //is main buf alloc or static
    uint16_t layer_i;       //current layer index
    uint16_t* layer_body;    //current layer body addr
}tm_mdl_t;
...
typedef struct{
    ...
    uint16_t  pad[4];        //top,bottom,left,right
...
/******************************* MODEL FUNCTION ************************************/
tm_err_t tm_load  (tm_mdl_t* mdl, const uint16_t* bin, uint16_t*buf, tm_cb_t cb, tm_mat_t* in);   //load model
...
/******************************* UTILS FUNCTION ************************************/
uint16_t TM_WEAK tm_fp32to8(float fp32);
float TM_WEAK tm_fp8to32(uint16_t fp8);
