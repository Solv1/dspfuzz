...
tm_err_t TM_WEAK tm_load  (tm_mdl_t* mdl, const uint16_t* bin, uint16_t*buf, tm_cb_t cb, tm_mat_t* in)
{
    ...
    mdl->cb         = (void*)cb;
    if(buf == NULL) {
        mdl->buf        = (uint16_t*)tm_malloc(mdl->b->buf_size);
    ...
    if(mdl->b->sub_size > 0) {
        mdl->subbuf = (uint16_t*)tm_malloc(mdl->b->sub_size);
...
tm_err_t TM_WEAK tm_preprocess(tm_mdl_t* mdl, tm_pp_t pp_type, tm_mat_t* in, tm_mat_t* out)
{
    ...
    case TMPP_UINT2INT:
        for(int i=0; i<in_size; i++)
            out->data[i] = ((mtype_t)(((uint16_t*)(in->data))[i]-128))<<UINT2INT_SHIFT;
        break;
#else
    case TMPP_UINT2FP01:
        for(int i=0; i<in_size; i++)
            out->data[i] = (((uint16_t*)(in->data))[i])/255.0;
        break;
    case TMPP_UINT2FPN11:
        for(int i=0; i<in_size; i++)
            out->data[i] = ((((uint16_t*)(in->data))[i])-128)/128.0;
        break;
#endif
