...
/**********************************************************************\
  ...
  calculated and converted to a uint16_t in the range 0-100, for all
...
static uint16_t g_tmp_image[TMP_IMAGE_SIZE];
static uint16_t g_dp[DP_SIZE];
...
void setup_brightness_lut(uint16_t **bp, int32_t thresh, int32_t form)
{
    int32_t k;
    float temp;
...
uint16_t median(uint16_t *in, int32_t i, int32_t j, int32_t x_size)
{
    int32_t p[8], k, l, tmp;
...
void enlarge(uint16_t **in, uint16_t *tmp_image, int32_t *x_size, int32_t *y_size, int32_t border)
{
    int32_t i, j;
...
int16_t susan_smoothing(int32_t three_by_three, uint16_t *in, float dt, int32_t x_size, int32_t y_size, uint16_t *bp)
{
    float temp;
    int32_t n_max, increment, mask_size,
        i, j, x, y, area, brightness, tmp, centre;
    uint16_t *ip, *dp, *dpt, *cp, *out = in,
                                 *tmp_image;
    int32_t total;
    ...
    // tmp_image = (uint16_t *) malloc( (x_size+mask_size*2) * (y_size+mask_size*2) );
    ...
    { /* large Gaussian masks */
        ...
        // printf("%d\n", n_max * n_max);
        // dp     = (uint16_t *)malloc(n_max*n_max);
        dp = g_dp;
        dpt = dp;
        temp = -(dt * dt);
...
int32_t susan_smooth(uint16_t * in)
{
    uint16_t *bp;
